...
1. If you just created your account, update line 5 of `terraform/modules/server/server.tf` to `VM.Standard.A2.Flex`. You will create a paid instance temporarily then downgrade to a free tier instance. Otherwise, you may get Out of Host Capacity error when creating.
...
1. Now go into the ansible directory and create hosts.ini and input the following
   ```ini
   [palworld_servers]
...
## Tips and Tricks
- If you upgrade to Pay as You Go, you can update your ocpus from 2 to 4 and memory from 12 to 24 (which allows you to have more players), but be careful as you may accidentally be charged.
